			...
			<form action="" id="manage_user">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						<b class="text-muted">Information Personnelle</b>
						<div class="form-group">
							<label for="" class="control-label">Prénom</label>
							<input type="text" name="firstname" class="form-control form-control-sm" required value="<?php echo isset($firstname) ? $firstname : '' ?>">
						</div>
						<div class="form-group">
							<label for="" class="control-label">Votre prénom</label>
					...
					<div class="col-md-6">
						...
						<b class="text-muted">Information d'identification du système</b>
						<?php if($_SESSION['login_type'] == 1): ?>
						<div class="form-group">
							<label for="" class="control-label">Role d'utilisateur</label>
							<select name="type" id="type" class="custom-select custom-select-sm">
								<option value="2" <?php echo isset($type) && $type == 2 ? 'selected' : '' ?>>Greffier</option>
						...
						<div class="form-group">
							<label class="label control-label">Mot de passe confirmé</label>
	...
	$('[name="password"],[name="cpass"]').keyup(function(){
		...
		if(cpass == '' ||pass == ''){
			...
			if(cpass == pass){
				...
				$('#pass_match').attr('data-status','2').html('<i class="text-danger">Mot de passe ne correspondant pas.</i>')
			}
		}
	})
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					$('#msg').html("<div class='alert alert-danger'>L'Email existe déjà.</div>");
