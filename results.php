	...
	<div class="card card-outline card-primary">
		...
		<div class="card-header">
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="./index.php?page=new_result"><i class="fa fa-plus"></i> Ajouter Nouveau</a>
			</div>
		</div>
	<?php endif; ?>
		<div class="card-body">
...
<script>
	...
	_conf("Etes-vous sure de supprimer ce résultat ?","delete_result",[$(this).attr('data-id')])
	})
	...
	$('.status_chk').change(function(){
		...
		$.ajax({
			...
			error:function(err){
				console.log(err)
				alert_toast("Quelque chose s'est mal passé lors d ela mise à jour du statut du résultat .",'erreur')
					$('#status_chk').attr('data-state-stats','error').bootstrapToggle('toggle')
					end_load()
			},
			success:function(resp){
				if(resp == 1){
					alert_toast("état du résultat mis à jour avec succés .",'succés')
					end_load()
				}else{
					alert_toast("Quelque chose s'est mal passé lors d ela mise à jour du statut du résultat.",'erreur')
					$('#status_chk').attr('data-state-stats','error').bootstrapToggle('toggle')
	...
	function delete_result($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Données supprimées avec succés",'succés')
					setTimeout(function(){
						location.reload()
					},1500)
