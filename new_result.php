	...
	<div class="card card-outline card-primary">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="d-flex justify-content-center align-items-center">
	            ...
	            <button class="btn btn-sm btn-primary bg-gradient-primary" type="button" id="add_mark">Ajouter</button>
            </div>
        </div>
    	<hr>
  	...
  	<div class="card-footer border-top border-info">
  		<div class="d-flex w-100 justify-content-center align-items-center">
  			<button class="btn btn-flat  bg-gradient-primary mx-2" form="manage-result">Enregistrer</button>
  			<a class="btn btn-flat bg-gradient-secondary mx-2" href="./index.php?page=results">Annuler</a>
  		</div>
  	</div>
	</div>
</div>
<script>
	...
	$('#add_mark').click(function(){
		...
		var mark = $('#mark').val()
		if(subject_id == '' && mark == ''){
			alert_toast("Veuillez sélectionner le sujet et entrer un point avant d'ajouter à la liste.","Erreur");
			return false;
		}
	...
	$('#manage-result').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast("Données enregistrées avec succés.","succés");
					setTimeout(function(){
              location.href = 'index.php?page=results'
					},2000)
				}else if(resp == 2){
          $('#msg').html('<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Le code Etudiant existe déjà.</div>')
          end_load()
        }
			}
		})
